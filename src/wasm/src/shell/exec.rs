...
impl Exec {
        ...
        for path in new_path.split("/").filter(|&x| !x.is_empty()) {
            ...
            if !found {
                let new_node = Rc::new(RefCell::new(Node::new(
                    path.to_string(),
                    NodeType::Directory(Directory::new(path.to_string(), Vec::new())),
    ...
    fn touch(current: &Rc<RefCell<Node>>, args: Vec<String>) -> String {
        ...
        let new_node = Rc::new(RefCell::new(Node::new(
            cloned_new_file,
            NodeType::File(File::new(new_file, " ".to_string())),
    ...
    fn write(current: &Rc<RefCell<Node>>, root: &Rc<RefCell<Node>>, args: Vec<String>) -> String {
        ...
        match file_node {
            Some(file_node) => match file_node.borrow().get_node_type() {
                NodeType::Directory(_) => {
                    return "Not a file".to_string();
                }
                NodeType::File(ref mut file) => file.write(content),
            },
    ...
    pub fn execute(
        ...
        for (i, statement) in statements.iter().enumerate() {
            ...
            output = match &statement.command[..] {
                "" => " ".to_owned(),
                "github" => "<a href=\"https://github.com/soohoonc/\" target=\"_blank\">github</a>"
                    .to_owned(),
